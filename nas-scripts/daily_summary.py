...
"""
    ...
    # 5. 生成每日点评
    insight = daily_insight(refined, api_key)
    if insight:
        md += f"""
...
"""
    ...
    # 6. 提取待办并合并到文件
    new_todos = extract_todos(raw_entries, api_key)
    if new_todos:
        all_todos = load_todos_file()
        for t in new_todos:
            entry = {"content": t["content"], "date": t.get("date", target_date.isoformat()), "done": False}
            if t["type"] == "long":
                all_todos["pending_long"].append(entry)
            else:
                all_todos["pending_day"].append(entry)
        save_todos_file(all_todos)
